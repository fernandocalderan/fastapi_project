...
-- ============================================================
-- seed_distributor_db_regen.sql (UTF-8)
-- One-shot: distributor_raw + public + sync + checks
-- PG 13+ compatible
-- ============================================================
BEGIN;
...
WITH brands AS (
  SELECT unnest(ARRAY[
    ...
    'UrbanBread Co','CampoOlive Oils','CrispBite Snacks','MeadowFresh Eggs','RedVine Wines',
...
-- ---------- seed: workers (20) ----------
TRUNCATE workers RESTART IDENTITY;
DO $$
DECLARE
  g INT;
  names TEXT[] := ARRAY[
...
BEGIN
  FOR g IN 1..array_length(names,1) LOOP
    INSERT INTO workers (
      ...
      names[g],
      concat('ID', to_char(20000000 + g, 'FM9999999')),
      ...
      (ARRAY['Indefinido','Temporal','Practicas','Indefinido'])[1 + (g % 4)],
...
SELECT
  concat((ARRAY['BlueStone','LaVida','CasaBella','MetroFoods','GreenFork','Sol y Mar','TerraCotta','UrbanEats','PrimeGrocer','NightOwl'])[1 + (g % 10)], ' ',
         (ARRAY['Restaurant','Bar','Hotel','Supermarket','Catering'])[1 + (g % 5)]) as company_name,
  ...
  lower( concat('contact', g, '@client.example.com') ),
  concat( (ARRAY['Miguel','Laura','Carlos','Ana','Jorge','Sofia','Diego','Elena','Pablo','Irene'])[1 + (g % 10)], ' ',
          (ARRAY['Gomez','Lopez','Silva','Fernandez','Martins'])[1 + (g % 5)] ),
...
BEGIN
  ...
  FOR i IN 1..1000 LOOP
    ...
    VALUES (
      ...
      concat('Quality ', lower( (categories[1 + (floor(random()*array_length(categories,1))::int)]) ), ' product'),
      ingr,
...
BEGIN
  FOR c IN 1..120 LOOP
    ...
    VALUES ((1 + (c % 500)), now() - (c * interval '1 day'), CASE WHEN c % 3 = 0 THEN 'Completed' ELSE 'Pending' END)
    ...
    FOR i IN 1..(3 + (random()*3)::int) LOOP
      ...
      FROM (
        SELECT id, unit_price, vat_rate FROM products ORDER BY random() LIMIT 1
      ) t;
...
-- ============================================================
-- switch to public and build canonical schema
-- ============================================================
...
CREATE TABLE IF NOT EXISTS shipments (
    ...
    delivery_status VARCHAR(40) DEFAULT 'en transito',
...
SELECT
    id, company_name, NULL, phone, email, address, city, country
FROM distributor_raw.providers
ORDER BY id;

INSERT INTO public.customers (id, name, contact_name, phone, email, address, city, country)
SELECT
    id, company_name, contact_person, phone, email, billing_address, city, country
FROM distributor_raw.clients
ORDER BY id;

INSERT INTO public.warehouses (name, address, city, manager_name)
VALUES
('Central Madrid Norte', 'Calle Logistica 12', 'Madrid', 'Ana Prieto'),
('Hub Barcelona Zona Franca', 'Av. del Puerto 455', 'Barcelona', 'Nil Fores'),
('Nodo Valencia Mediterraneo', 'Poligono Safor 77', 'Valencia', 'Clara Llopis'),
('Centro Sevilla Atlantico', 'Carretera Cadiz km 9', 'Seville', 'Rafael Moya'),
('Deposito Bilbao Norte', 'Parque Industrial Ibaizabal 5', 'Bilbao', 'Uxue Aguirre'),
('Plataforma Lisboa Tejo', 'Rua do Porto 210', 'Lisbon', 'Margarida Sousa')
ON CONFLICT DO NOTHING;

INSERT INTO public.categories (name, description)
SELECT DISTINCT
    COALESCE(NULLIF(category, ''), 'Sin categoria') AS name,
    'Generada desde distributor_raw.products'
...
LEFT JOIN public.categories c
    ON c.name = COALESCE(NULLIF(p.category, ''), 'Sin categoria')
ORDER BY p.id;
...
SELECT
    ...
    o.order_date::date,
    NULL,
    CASE WHEN lower(o.status) LIKE 'completed%' THEN 'completado'
         WHEN lower(o.status) LIKE 'pending%' THEN 'pendiente'
         ELSE o.status END,
    o.total_gross
...
order_schedule AS (
    SELECT
        ...
        CASE
            ...
            WHEN (o.id % 3) = 0 THEN 'en transito'
            ELSE 'entregado'
...
BEGIN
    ...
    RAISE NOTICE 'Totals => customers: %, orders: %, order_items: %, products: %, suppliers: %, warehouses: %, inventories: %, shipments: %',
