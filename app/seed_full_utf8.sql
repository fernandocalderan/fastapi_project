...
-- Work inside a dedicated schema
...
-- PRODUCTS (catalog) â€” includes traceability fields and VAT
...
-- âœ… Seed providers (50) â€” fix brand selection (no cartesian explosion)
WITH brands AS (
  SELECT unnest(ARRAY[
...
SELECT
  concat(b.brand, ' Ltd.') AS company_name,
  gen_vat('EU', g.g) AS vat_number,
  concat((g.g % 200)+1, ' Supplier Ave') AS address,
  (ARRAY['Barcelona','Madrid','Valencia','Seville','Bilbao','Lisbon','Porto','Paris','Milan','Berlin'])[1 + (g.g % 10)],
  (ARRAY['Spain','Spain','Spain','Spain','Portugal','Portugal','France','Italy','Germany','Netherlands'])[1 + (g.g % 10)],
  concat('+34', (600000000 + (g.g * 31))::text),
  lower(replace(b.brand,' ','') || '@supplier.example.com'),
  (ARRAY['Alice Green','Mark Stone','Sofia Rossi','Luis Garcia','Emma Moreira'])[1 + (g.g % 5)],
  (ARRAY['Food','Beverage','Packaging','Logistics','Ingredients'])[1 + (g.g % 5)],
  (ARRAY['ISO 22000','BRC','IFS','Organic Cert','HACCP'])[1 + (g.g % 5)],
  (current_date - (g.g % 365))::date,
  'Active'
FROM g
CROSS JOIN LATERAL (
  SELECT brand
  FROM brands
  ORDER BY brand
  OFFSET ((g.g-1) % (SELECT count(*) FROM brands)) LIMIT 1
) AS b;

-- WORKERS
DROP TABLE IF EXISTS public.workers CASCADE;
DROP TABLE IF EXISTS distributor_raw.workers CASCADE;

CREATE TABLE distributor_raw.workers (
  id SERIAL PRIMARY KEY,
  full_name VARCHAR(120) NOT NULL,
  national_id VARCHAR(30) UNIQUE NOT NULL,
  role VARCHAR(80),
...
-- ðŸ§‘â€ðŸ­ Seed workers (20) â€” versiÃ³n corregida sin duplicados
-- Limpieza segura antes de insertar
...
DECLARE
  g INT;
  names TEXT[] := ARRAY[
    'Fernando Aringhieri','Lilian Aguiar','Carlos Mendes','Sara Lopez','Miguel Santos',
    'Anna Keller','John Smith','Emily Clark','Oliver Brown','Sofia Martins',
    'Luis Rodriguez','Marta Diaz','Pablo Ruiz','Clara Iglesias','David Perez',
    'Nora Silva','Hugo Fernandes','Laura Gomez','Iker Martinez','Elena Torres'
  ];
BEGIN
  FOR g IN 1..array_length(names,1) LOOP
    INSERT INTO workers (
      full_name, national_id, role, department, birth_date, hire_date, gross_annual_salary,
      contract_type, phone, email, address, city, postal_code, country, status
    ) VALUES (
      names[g],
      concat('ID', to_char(20000000 + g, 'FM9999999')), -- ðŸ” cambiado el prefijo base
      (ARRAY['Logistics Manager','Warehouse Operative','Sales Executive','Accountant',
              'Procurement Specialist','Driver','Quality Technician','Customer Success',
              'IT Specialist','HR Manager'])[1 + (g % 10)],
      (ARRAY['Logistics','Operations','Sales','Finance','Procurement','Transport','Quality',
              'Customer','IT','HR'])[1 + (g % 10)],
      (date '1980-01-01' + (g * 500) * interval '1 day')::date,
      (current_date - (g * 25))::date,
      (22000 + (g % 10) * 1400)::numeric(12,2),
      (ARRAY['Indefinido','Temporal','PrÃ¡cticas','Indefinido'])[1 + (g % 4)],
      concat('+34', (610000000 + g)::text),
      lower(replace(split_part(names[g], ' ', 1), ' ', '')) || '@distributor.example.com',
      concat((10 + g % 90), ' Industrial Park'),
      (ARRAY['Barcelona','Madrid','Valencia','Seville','Bilbao'])[1 + (g % 5)],
...
TRUNCATE distributor_raw.workers RESTART IDENTITY CASCADE;

INSERT INTO distributor_raw.workers (
  full_name, national_id, role, department, birth_date, hire_date,
  gross_annual_salary, contract_type, phone, email, address, city, postal_code, country, status
)
SELECT
  (ARRAY[
    'Fernando Aringhieri','Lilian Aguiar','Carlos Mendes','Sara Lopez','Miguel Santos',
  ...
  concat('ID', to_char(10000000 + g * 19 + floor(random()*999)::int, 'FM9999999')) AS national_id,
  (ARRAY[
    'Logistics Manager','Warehouse Operative','Sales Executive','Accountant','Procurement Specialist',
    'Driver','Quality Technician','Customer Success','IT Specialist','HR Manager'
  ])[1 + (g % 10)] AS role,
  (ARRAY[
    'Logistics','Operations','Sales','Finance','Procurement',
    'Transport','Quality','Customer','IT','HR'
  ])[1 + (g % 10)] AS department,
  ...
  (ARRAY['Indefinido','Temporal','Indefinido','PrÃ¡cticas'])[1 + (g % 4)] AS contract_type,
  concat('+34', (610000000 + g)::text) AS phone,
  lower(replace((
    ARRAY['fernando','lilian','carlos','sara','miguel','anna','john','emily','oliver','sofia',
           'luis','marta','pablo','clara','david','nora','hugo','laura','iker','elena']
  )[g], ' ', '')) || '@distributor.example.com' AS email,
...
BEGIN
  ...
  FOR i IN 1..1000 LOOP
    ...
    base_cost := round( ( (0.5 + random() * 25) )::numeric, 2);
    margin := round( (10 + random() * 120)::numeric, 2);
...
-- Switch back to public schema and build canonical tables
...
CREATE TABLE IF NOT EXISTS shipments (
    ...
    delivery_status VARCHAR(40) DEFAULT 'en trÃ¡nsito',
...
FROM (
    VALUES
        ('Central Madrid Norte', 'Calle LogÃ­stica 12', 'Madrid', 'Ana Prieto'),
        ('Hub Barcelona Zona Franca', 'Av. del Puerto 455', 'Barcelona', 'Nil ForÃ©s'),
        ('Nodo Valencia MediterrÃ¡neo', 'PolÃ­gono Safor 77', 'Valencia', 'Clara Llopis'),
        ('Centro Sevilla AtlÃ¡ntico', 'Carretera CÃ¡diz km 9', 'Sevilla', 'Rafael Moya'),
        ('DepÃ³sito Bilbao Norte', 'Parque Industrial Ibaizabal 5', 'Bilbao', 'Uxue Aguirre'),
...
SELECT DISTINCT
    COALESCE(NULLIF(category, ''), 'Sin categorÃ­a') AS name,
    'CategorÃ­a generada a partir del dataset completo'
...
LEFT JOIN public.categories c
    ON c.name = COALESCE(NULLIF(p.category, ''), 'Sin categorÃ­a')
ORDER BY p.id;

-- Some inventories across warehouses for first ~200 products
WITH wh AS (
...
SELECT
    product_id, warehouse_id, qty, GREATEST(30, (qty / 4))::int, restocked
FROM combinations
...
SELECT
    o.id, o.client_id, o.order_date::date, NULL, o.status, o.total_gross
...
SELECT
    oi.id, oi.order_id, oi.product_id, oi.quantity, oi.unit_price_net, 0
...
order_schedule AS (
    SELECT
        ...
        CASE
            ...
            WHEN (o.id % 3) = 0 THEN 'en trÃ¡nsito'
            ELSE 'entregado'
...
SELECT
    id, warehouse_id, shipped_at, estimated_delivery, delivery_status,
    concat('TRK-', lpad(id::text, 7, '0'))
FROM order_schedule
ORDER BY id;
...
BEGIN
    ...
    IF customers_count = 0 OR orders_count = 0 OR products_count = 0 OR suppliers_count = 0 THEN
        RAISE EXCEPTION 'Las tablas principales quedaron vacÃ­as (customers/orders/products/suppliers). Revisa los mensajes anteriores.';
    END IF;

    IF order_items_count = 0 THEN
        RAISE EXCEPTION 'La tabla order_items no recibiÃ³ datos. Revisa la generaciÃ³n de pedidos.';
    END IF;

    IF warehouses_count = 0 OR inventories_count = 0 OR shipments_count = 0 THEN
        RAISE EXCEPTION 'Los datos logÃ­sticos no se poblaron (warehouses/inventories/shipments).';
    END IF;

    RAISE NOTICE 'Canonical totals â€” customers: %, orders: %, order_items: %, products: %, suppliers: %, warehouses: %, inventories: %, shipments: %',
        customers_count, orders_count, order_items_count, products_count, suppliers_count, warehouses_count, inventories_count, shipments_count;
END $$;

-- Quick checks:
-- SELECT count(*) FROM distributor_raw.providers; -- ~50
-- SELECT count(*) FROM distributor_raw.workers;   -- 20
-- SELECT count(*) FROM distributor_raw.clients;   -- 500
-- SELECT count(*) FROM distributor_raw.products;  -- 1000
